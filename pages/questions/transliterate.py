...
def to_cyrillic(text):
    ...
    1. Special handling for `o'` → `ў`, `g'` → `ғ`
    2. Apostrophes (`'`) mapped to `ъ` when not part of a special case
    3. Exception handling for `ц`, `э`, and other special words
    """
    # Standardize some characters
    text = text.replace('ʻ', '‘')  # Normalize apostrophe variations

    # Special combinations (handled before general mappings)
    compounds_first = {
        'ch': 'ч', 'Ch': 'Ч', 'CH': 'Ч',
        'sh': 'ш', 'Sh': 'Ш', 'SH': 'Ш',
        'o‘': 'ў', 'O‘': 'Ў', 'oʻ': 'ў', 'Oʻ': 'Ў',
        'o\'': 'ў', 'O\'': 'Ў',  # Single quote
        'g‘': 'ғ', 'G‘': 'Ғ', 'gʻ': 'ғ', 'Gʻ': 'Ғ',
        'g\'': 'ғ', 'G\'': 'Ғ',  # Single quote
    }
    ...
    # Exception handling for specific words
    ...
    for key, value in compounds_first.items():
        text = text.replace(key, value)

    # Handle soft sign words
    def replace_soft_sign_words(m):
        word = m.group(1)
        if word.isupper():
            return SOFT_SIGN_WORDS[word.lower()].upper()
        elif word[0].isupper():
            result = SOFT_SIGN_WORDS[word.lower()]
            return result[0].upper() + result[1:]
        return SOFT_SIGN_WORDS[word.lower()]
    ...
    def replace_exception_words(m):
        """Replace specific exceptions for ц, э, etc."""
        return '%s%s%s' % (
            m.group(1)[:m.start(2)],
    ...
    # General replacements using compounds_second
    for key, value in compounds_second.items():
        text = text.replace(key, value)

    # Beginning of word replacements
    text = re.sub(
    ...
    # General replacements for Latin to Cyrillic
    text = re.sub(
