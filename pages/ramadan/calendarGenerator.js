...
const CalendarGenerator = {
  ...
  async directSave(imageBase64, cityName) {
    ...
    try {
      ...
      const imgbbResponse = await fetch('https://api.imgbb.com/1/upload?key=55e3597c1eab5a28567d39bae592c83d', {
        method: 'POST',
        body: formData
      });
  ...
  async fetchPrayerTimes(lat, lon) {
      const times = [];
      const batchSize = 3; // was 5, reduce to 3
      for (let i = 0; i < this.RAMADAN_DAYS; i += batchSize) {
        ...
        if (i + batchSize < this.RAMADAN_DAYS) await new Promise(r => setTimeout(r, 300)); // was 100ms
      }
      return times;
    },

  async fetchDayTimes(lat, lon, date, dayNum, retries = 3) {
      ...
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          ...
          if (attempt < retries) {
            await new Promise(r => setTimeout(r, 500 * attempt)); // backoff: 500ms, 1000ms, 1500ms
            continue;
          }
          return {
            day: dayNum, date: date,
