...
const CalendarGenerator = {
  ...
  setupResultPageEvents(resultPage, imageBase64, cityName) {
    ...
    resultPage.querySelector('#saveImageBtn').addEventListener('click', () => {
      this.haptic('medium');
      try {
        const htmlContent = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Ramazon 2026 - ${cityName}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{min-height:100vh;background:linear-gradient(180deg,#0a1628 0%,#132743 100%);display:flex;flex-direction:column;align-items:center;padding:20px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif}h1{color:#f4c542;font-size:1.3rem;margin-bottom:8px;text-align:center}.hint{color:#10b981;font-size:0.95rem;margin-bottom:20px;text-align:center;padding:12px 20px;background:rgba(16,185,129,0.15);border-radius:10px}img{max-width:100%;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.5)}.footer{margin-top:20px;color:#64748b;font-size:0.8rem}</style></head><body><h1>📅 Ramazon 2026 Taqvimi</h1><p class="hint">👆 Rasmni bosib turing → "Rasmni saqlash"</p><img src="${imageBase64}" alt="Ramazon 2026 Taqvimi - ${cityName}" /><p class="footer">@muslim_vegukin_bot</p></body></html>`;
        const blob = new Blob([htmlContent], { type: 'text/html' });
        const blobUrl = URL.createObjectURL(blob);
        if (tg?.openLink) tg.openLink(blobUrl);
