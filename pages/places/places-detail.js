  ...
  detailContent.innerHTML = `
    <div class="detail-card">
      ...
      <div class="detail-content">
        ...
        ${PLACE_TYPE === 'restaurant' && place.menuUrl ? `
          <a href="${place.menuUrl}" class="menu-cta-card" onclick="openMenuApp(event, '${place.menuUrl}')">
            <div class="menu-cta-icon">📋</div>
            <div class="menu-cta-body">
              <h4 class="menu-cta-title">${window.I18N ? I18N.t('detail.menuCtaTitle') : 'Menyu va onlayn buyurtma'}</h4>
              <p class="menu-cta-sub">${window.I18N ? I18N.t('detail.menuCtaSub') : 'Stoldan yoki olib ketish uchun buyurtma bering'}</p>
            </div>
            <div class="menu-cta-arrow">→</div>
          </a>
        ` : ''}
...
function openMenuApp(e, menuUrl) {
  e.preventDefault();
  try {
    if (window.Telegram?.WebApp?.HapticFeedback) {
      window.Telegram.WebApp.HapticFeedback.impactOccurred('light');
    }
    window.Telegram.WebApp.openTelegramLink(menuUrl);
  } catch (err) {
    window.open(menuUrl, '_blank');
  }
}

function openAdminContact(e) {
  e.preventDefault();
  try {
    window.Telegram.WebApp.openTelegramLink('https://t.me/otabeksattarov');
  } catch (err) {
