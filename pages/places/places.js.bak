...
// ============================================
const MENU_API_BASE = 'https://vegukinshop.ddns.net';
...
// Extract shop_id from a Telegram menu URL:
//   https://t.me/muslim_vegukin_bot/shop?startapp=toshkent_halal
function extractShopIdFromMenuUrl(menuUrl) {
  if (!menuUrl) return null;
  try {
    const url = new URL(menuUrl);
    return url.searchParams.get('startapp') || null;
  } catch (e) { return null; }
}
...
const hasMenu = !!(place.menuUrl && place.buildingType === 'Oshxona');
    if (hasMenu) card.classList.add('has-menu');
    const shopIdForMenu = hasMenu ? extractShopIdFromMenuUrl(place.menuUrl) : null;
    const menuLabel    = window.I18N ? I18N.t('places.menuAvailable') : 'Menyu mavjud';
    ...
    card.innerHTML = `
      ...
      ${hasMenu && shopIdForMenu ? `
        <div class="menu-strip" data-menu-strip data-shop-id="${shopIdForMenu}">
          <div class="menu-strip-header">
            <div class="menu-strip-title">
          ...
          <div class="menu-items-scroll">
            ${[0,1,2,3].map(() => `
              <div class="menu-item-chip skeleton">
                <div class="menu-item-chip-image"></div>
                <div class="menu-item-chip-info">
                  <p class="menu-item-chip-name">&nbsp;</p>
                  <p class="menu-item-chip-price">&nbsp;</p>
                </div>
              </div>
            `).join('')}
          </div>
  ...
  updatePageTitle();
  loadingIndicator.style.display = 'none';
  // Lazy-load menu preview strips for restaurants with menuUrl — non-blocking
  loadMenuStrips();
}


function loadMenuStrips() {
  document.querySelectorAll('[data-menu-strip]').forEach(stripEl => {
    const shopId = stripEl.getAttribute('data-shop-id');
    if (shopId) fetchAndInjectStrip(shopId, stripEl);
  });
}

async function fetchAndInjectStrip(shopId, stripEl) {
  try {
    const resp = await fetch(
      `${MENU_API_BASE}/shop/${encodeURIComponent(shopId)}/api/restaurant/preview-dishes?limit=4`,
      { signal: AbortSignal.timeout(8000) }
    );
    if (!resp.ok) throw new Error('Preview fetch failed: ' + resp.status);
    const data = await resp.json();
    renderStripDishes(stripEl, shopId, data.dishes || [], data.total || 0);
  } catch (err) {
    console.warn('Menu strip failed for', shopId, err);
    stripEl.style.display = 'none';  // fail silently
  }
}

function renderStripDishes(stripEl, shopId, dishes, total) {
  if (!dishes.length) { stripEl.style.display = 'none'; return; }
  const scrollEl = stripEl.querySelector('.menu-items-scroll');
  if (!scrollEl) return;

  const moreText = window.I18N ? I18N.t('places.viewAllDishes') : 'Hammasini ko\'rish';
  const noPhoto = '🍽️';

  const chipsHTML = dishes.map(d => {
    const photoHTML = d.image_path
      ? `<img src="${MENU_API_BASE}/shop/${encodeURIComponent(shopId)}/uploads/${encodeURIComponent(d.image_path)}" alt="${escapeHtmlSimple(d.name)}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=&quot;menu-item-chip-image-fallback&quot;>${noPhoto}</div>'" />`
      : `<div class="menu-item-chip-image-fallback">${noPhoto}</div>`;
    const priceFormatted = (d.price != null)
      ? `${Math.round(d.price).toLocaleString()} ₩`
      : '';
    return `
      <div class="menu-item-chip">
        <div class="menu-item-chip-image">${photoHTML}</div>
        <div class="menu-item-chip-info">
          <p class="menu-item-chip-name">${escapeHtmlSimple(d.name)}</p>
          <p class="menu-item-chip-price">${priceFormatted}</p>
        </div>
      </div>
    `;
  }).join('');

  const moreChip = (total > dishes.length) ? `
    <div class="menu-item-chip more-chip">
      <span class="more-icon">→</span>
      <span class="more-text">${moreText}</span>
    </div>
  ` : '';

  scrollEl.innerHTML = chipsHTML + moreChip;
}
